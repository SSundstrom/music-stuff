...
import { advanceToNextTurn, getGuessState } from "@/lib/guess-game";
...
export async function POST(
  ...
  try {
    ...
    if (session.ownerId !== authSession.user.id) {
      return new Response(
        JSON.stringify({ error: "Only the owner can advance turns" }),
        { status: 403, headers: { "Content-Type": "application/json" } },
      );
    }

    const result = await advanceToNextTurn(sessionId);
