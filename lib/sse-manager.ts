...
class SSEManager {
  ...
  removeConnection(sessionId: string, controller: ReadableStreamDefaultController<Uint8Array>): void {
  ...
  broadcast(sessionId: string, message: SSEMessage): void {
    ...
    console.log(`[SSE] Broadcasting ${message.type} to ${connections.length} clients in session ${sessionId}`);
    ...
    for (let i = 0; i < connections.length; i++) {
      ...
      try {
        ...
        console.error("[SSE] Error sending message to client, removing connection:", error);
    ...
    for (let i = 0; i < connections.length; i++) {
      ...
      if (connection.playerId === playerId) {
        try {
          ...
          console.error("[SSE] Error sending message to player, removing connection:", error);
          deadConnections.unshift(i);
        }
      }
    }
